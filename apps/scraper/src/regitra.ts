...
export class Regitra {
  ...
  async start(browser: Browser) {
    log.info("Starting regitra.");
    const autos = await this.db.getAutosForLookup();
    if (!autos || autos.length === 0) {
      log.info("No vechiles need lookup.");
      return;
    }
    ...
    try {
      ...
      for (const plate of plates) {
        ...
        if (
          ...
          return { ...data, plates: [plate], success: true };
        }
      }
    } catch (error) {
      ...
      while (retry < 3) {
        ...
        try {
          ...
          const response = await fetch(
            "https://www.eregitra.lt/clients-self-service-backend/public/vehicle/registration-data",
            {
              method: "POST",
              headers: {
              ...
              body: JSON.stringify({
                ...
                vin,
                plateNo: plate,
                technicalInspectionChecked: true,
                civilInsuranceChecked: true,
                wantedChecked: true,
          ...
          if (errorData.success) {
            ...
            if (errorData.data.message.includes("INVALID_RECAPTCHA")) {
              ...
              throw new Error(
                `Got unknown error from regitra insurance. Error: ${errorData.data.message}`
              );
            }
          }

          const data = await regitraInsurance.parseAsync(responseJson);
          return {
            vehicleMake: data.vehicleMake,
            vehicleModel: data.vehicleCommercialName,
            statusInRegister:
              data.statusCode === "R"
                ? data.trafficParticipationStatus
                  ? "Įregistruota"
